...
1. **Take the inventory.** Run `orch ps`. Without a filter it lists everything still active plus the ten
   most recently finished tasks, and it reports the process status and the report status in separate
   columns — a task can exit cleanly while its own report says it failed. `orch ps --status running` and
   friends narrow it; the known statuses are `pending`, `running`, `succeeded`, `failed`, `cancelled`,
   `timed_out`.

2. **Look closer at whatever is puzzling.** `orch logs <id>` shows the normalized events;
   `orch logs <id> --raw` shows the provider's own output, which is where a silent failure usually
...
3. **Take a snapshot of everything in flight** with `orch watch --once` — one frame, then exit. Never run
   `orch watch` without `--once`: it is an interactive terminal view that redraws and does not
   terminate.
...
5. **Cancel what should die.** `orch cancel <id>` on anything hung, superseded, or running against an
   objective that no longer matters. Say what is being cancelled and why before doing it. A task killed
   outright elsewhere may still read as `running`; that reconciles itself on read, and `orch gc` both
...
- Don't run `orch watch` without `--once`.
- Don't cancel a task just because it is slow; check what its logs say first.
- Don't run `orch gc` as a reflex — it removes the worktrees of finished tasks, and their diffs with
  them. Use `orch gc --dry-run` first, and never `--force` without being asked.
