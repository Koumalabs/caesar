...
3. **Wait for the result.** Call `caesar_await` with that `task_id`. A `pending: true` result just means the review needs more time: call `caesar_await` again. Use `caesar_status` for a lightweight check in between, and `caesar_logs` if the final report looks incomplete or surprising.
