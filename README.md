...
`caesar run` accepts `--role <name>` (picks the agent via a configured role and its fallback chain — see `caesar role list`) or `--agent <id>` (fixes the agent, wins over `--role`), `--mode read-only|write`, `--isolation inplace|worktree|auto`, `--timeout 10m`, `--model <id>`, `--context <text or @file>`, `--network auto|on|off` (see "Supported agents" above), and `--channel` (opens the bidirectional MCP return channel: the sub-agent can ask the main agent a question along the way rather than guess — see `docs/protocol.md`). At least one of `--agent`/`--role` is required. Without `--model`, the role's `model` applies if the role declares one, then the agent's default from the `[models]` configuration table (`caesar agents set-model`) — an explicit `--model` always wins, and is refused outright on an agent that cannot choose a model rather than silently ignored.
...
**Choosing models.** The `[models]` table gives each agent a default model, and a `[[role]]` can carry its own `model` — the resolution order is explicit `--model`/`model:`, then the role's, then the elected agent's `[models]` entry, then the provider's own default:
...
Merged key by key across layers like the fields of `[policy]` — a project layer that pins `codex` says nothing about the other agents. `caesar agents set-model <id> <model>` / `unset-model <id>` write it, `caesar agents list` shows it (with `(ignored)` when the agent cannot choose a model — the delegation then drops the default with a report finding instead of failing), and the `caesar config` TUI edits it on the Agents (`m`) and Roles (`Model` field) tabs.
