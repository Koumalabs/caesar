...
title: Specification
sidebar_position: 2
description: The full OACP specification — task.json, report.json, events.jsonl, the four report-recovery tiers, the optional MCP return channel, and versioning.
---
...
Two points on `tool_use`, which decide what an observer ([`caesar watch`](../guides/watch.md)) can show of a running task:
...
`ask_orchestrator` drops the question into `questions/<id>.json` then waits for `answers/<id>.json` to appear (polling), at most 5 minutes by default and never beyond the time remaining on the task's `deadline_ms`. Without an answer within that window, the call returns normally — it is not an error — with an invitation to proceed on the agent's best judgment rather than waiting indefinitely. On the orchestrator side, answering is symmetrical: the [`caesar_answer`](../reference/mcp-tools.md) tool of the main MCP server (`@caesar/mcp-server`, outside the scope of this standard but provided by the reference implementation) writes `answers/<id>.json`; answering an unknown or already-answered question fails explicitly rather than writing silently.
...
- [Overview](./overview.md) — the contract condensed to what a sub-agent implementer needs first.
- [MCP tools](../reference/mcp-tools.md) — the ten tools that expose this standard from the orchestrator's side.
- [Custom agents](../guides/custom-agents.md) — wiring a CLI outside the catalogue against this contract.
