/**
 * `orch_logs` : un extrait des événements normalisés d'une tâche, ou de la
 * sortie brute de son CLI — le détail que `orch_status`/`orch_await` gardent
...
export const orchLogsDescription =
  ...
  "out, or produced a surprising report — orch_status and orch_await deliberately omit this level of detail to " +
...
export const orchLogsInputShape = {
...
const OrchLogsInputSchema = z.object(orchLogsInputShape);
export type OrchLogsInput = z.infer<typeof OrchLogsInputSchema>;
...
export async function orchLogs(session: McpSession, input: OrchLogsInput): Promise<CallToolResult> {
...
export function registerOrchLogs(server: McpServer, session: McpSession): void {
  server.registerTool(ORCH_LOGS, { description: orchLogsDescription, inputSchema: orchLogsInputShape }, (args) => orchLogs(session, args));
}
